...
#if defined(NDEBUG)
# error "ARGUSCOIN cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbf4507c725903904bc9f0b79b5474b3d18511d1f72ca4438cbcfbebcf741f429");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ARGUSCOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ARGUSCOIN Signed Message:\n";
...
{
    // ARGUSCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 14 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 5162927 * COIN;
            }
...
static const int64 nTargetTimespan = 40 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ARGUSCOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ARGUSCOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1488204909, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Argus is here ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 14 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04455fd5d21ca1b00424db8a2bdedb6c33465a1578e4e2f5ccadea1c4aef01aaac66c8f8388091d0bb81d754f400c102b90f5ca3ef48481c02a2faa924e8a6c0d7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1488204909;
        ...
        assert(block.hashMerkleRoot == uint256("0xfe54952ae274775495445ba636fb6bd98d38e4271bb86ed9009872c1dae1ae4e"));
...
unsigned char pchMessageStart[4] = { 0x18, 0xe5, 0x0f, 0x48 }; // ARGUSCOIN: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ARGUSCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ARGUSCOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ARGUSCOINMiner(CWallet *pwallet)
{
    printf("ARGUSCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("arguscoin-miner");
    ...
    unsigned int nExtraNonce = 0;
        ...
        printf("Running ARGUSCOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ARGUSCOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ARGUSCOINMiner, pwallet));
}
